...
class WorkspaceViewIssuesViewSet(BaseViewSet):
    ...
    def list(self, request, slug):
        ...
        # Group by (kanban / grouped list at workspace level). Grouping by a
        # per-project field such as ``state`` is meaningless across projects,
        # so the frontend only offers workspace-wide fields (state group,
        # priority, project, labels, assignees, ...).
